    ...
    states: {
      ...
      creatingIntCA: {
        invoke: {
          id: 'dfspIntCACreate',
          src: (ctx, event) => {
            console.log('createMachine v2 event', event)
            console.log('createMachine v3 event', opts)
            return invokeRetry({
              id: 'dfspIntCACreate',
              logger: opts.logger,
              retryInterval: opts.refreshIntervalSeconds * 1000,
              service: async () => opts.vault.createCA(opts.caCsrParameters),
            })
          },
          onDone: {
            target: 'uploadingToHub',
      ...
      creatingExtCA: {
        invoke: {
          id: 'dfspExtCACreate',
          src: (ctx, event) => {
            console.log('createMachine v4 event', opts)
            return invokeRetry({
              id: 'dfspExtCACreate',
              logger: opts.logger,
              retryInterval: opts.refreshIntervalSeconds * 1000,
        ...
        invoke: {
          ...
          src: (ctx) =>
            invokeRetry({
              ...
              retryInterval: opts.refreshIntervalSeconds * 1000,
              service: async () =>{
                console.log('createMachine v5 event', ctx)
