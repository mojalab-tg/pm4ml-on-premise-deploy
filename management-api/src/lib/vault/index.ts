...
class Vault {
  ...
  async createCA(subject: Subject) {
    ...
    console.log("createCA error")
    try {
      await this.deleteCA();
      assert(this.client);
      console.log("createCA root subject subject", subject)
      ...
      console.log("createCA created", data.certificate)
      return {
        cert: data.certificate,
        key: data.private_key,
      };
    } catch (e) {
      console.log("createCA error", e)
    }
  }
  ...
  async getCA() {
    try {
      assert(this.client);
      console.log('getCA getCA')
      return this.client.request({
        path: `/${this.cfg.mounts.pki}/ca/pem`,
        method: 'GET',
      });
    } catch (e) {
      console.log('getCA error', e)
    }
  }

  async createDFSPServerCert(csrParameters: CsrParams) {
    console.log('createDFSPServerCert', csrParameters)
    ...
    console.log('createDFSPServerCert V1', data)
    return {
      intermediateChain: data.ca_chain[0], // custom
  ...
  async signHubCSR(csr: string) {
    assert(this.client);
    console.log('signHubCSR', this.cfg)
    const options = {
  ...
  createCSR(csrParameters?: CsrParams) {
    ...
    // }
     ...
     // Add subjectAltName as an attribute
  
    // Ensure valid IPs and add subjectAltName as an attribute
    // if (csrParameters?.extensions?.subjectAltName) {
    //   const { dns, ips } = csrParameters.extensions.subjectAltName;
    ...
    //       || /^[0-9a-fA-F:]+$/.test(ip); // Matches IPv4 and IPv6
    //   });
    ...
    //       altNames: [
    //         ...(dns?.map?.((value) => ({ type: SubjectAltNameType.DNS, value })) || []),
    //         ...(validIps?.map?.((value) => ({ type: SubjectAltNameType.IP, value })) || []),
    //       ]
    //     }]
    //   }]);
    // }
